...
class Server:
    ...
    @app.get(
        '/bot/api/ping', 
        responses={
            501: {'model' : ErrorResponse, 'description' : 'Access denied'},
            200: {'model' : InfoResponse, 'description' : 'Pong!'}
        }
    )
    async def ping(api_key: str):
        if api_key != _config.INTERNAL_API_KEY:
            return JSONResponse(ErrorResponses.access_denied.model_dump(), status_code=501)
...
if __name__ == '__main__':
    uvicorn.run(Server().app, host='blitzhub.eu', port=8000)