...
_log = logger.get_logger(__name__, 'ImageCommonLogger', 'logs/image_common.log')
    ...
    def draw_rating_points(self, img: Image.Image):
        for i in self.coord.rating_stats_point.keys():
            img.text(
                ...
                font=self.fonts.point,
                anchor='mm',
                fill=self.point_coloring(i, getattr(self.data.rating, i), rating=True)
            )
    ...
    def darw_common_points(self, img: Image.Image):
        for i in self.coord.common_stats_point.keys():
            img.text(
                ...
                font=self.fonts.point,
                anchor='mm',
                fill=self.point_coloring(i, getattr(self.data.all, i), rating=True)
            )
